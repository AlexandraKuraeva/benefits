  ...
  try {
    const [invoiceCount, customerCount, invoiceStatus] = await Promise.all([
      ...
      supabase
        .from('invoices')
        .select(
          'sum(CASE WHEN status = \'paid\' THEN amount ELSE 0 END) as paid, sum(CASE WHEN status = \'pending\' THEN amount ELSE 0 END) as pending'
        )
        .then(({ data }) => data[0]),
    ]);
  ...
  try {
    const { data, error } = await supabase
      ...
      .join(
        ...
        'customer_id',
        'id',
        { foreignTable: 'invoices', select: 'amount, status' }
      )
